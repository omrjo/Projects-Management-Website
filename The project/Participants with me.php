...
$database=mysqli_connect("localhost","root","","rpms");
                    ...
                    $query_id_proposal="SELECT id_proposal FROM researcher_has_proposal WHERE id_researcher=$id_researcher";
                    $result_id_proposal=mysqli_query($database , $query_id_proposal);
                    $id_proposal= $result_id_proposal->fetch_assoc();
                    $id_proposal=$id_proposal['id_proposal'];

                    $query22="SELECT * FROM researchers JOIN researcher_has_proposal ON researchers.id_researcher=researcher_has_proposal.id_researcher WHERE id_proposal=$id_proposal ";             
...
<center>
	<ul>
		...
		echo "<li><a href='' class='s1'>".$row['f_name']." ".$row['l_name']."</a>
			        <a href='' class='sp'> 
					<div style='width:100px;'>
				     ".$row['id_proposal']."</div></a></li><hr>";  } ?>
